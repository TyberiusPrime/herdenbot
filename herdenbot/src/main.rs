...
static LAST_DECRYPT: LazyLock<Mutex<String>> = LazyLock::new(|| Mutex::new("".to_string()));

async fn handle_decrypt_http_request(
    bot2: Arc<teloxide::Bot>,
    ...
    let mut send = false;
    let msg = format!("Decrypt requested. Result was: {do_decrypt}");
    {
        let mut last = LAST_DECRYPT.lock().unwrap();
        if msg != *last {
            *last = msg.clone();
            send = true;
        }
    ...
    if send {
        bot2.send_message(chat_id, msg).await.unwrap();
    }

    let key = SECRETDATA
        .get()
        .and_then(|x| x["zfs_keys"].as_object())
        .and_then(|x| x["sami"].as_str())
        .expect("No key found")
        .to_string();
    if do_decrypt {
...
async fn main() {
    ...
    let secret_file =
        std::env::var("SECRET_FILE").unwrap_or_else(|_| "/secrets/herdenbot/bot.json".to_string());
    ...
    let hello = warp::path!("sami")
        .and(warp::get())
        .and(bot_filter)
        .and_then(handle_decrypt_http_request);
        ...
        BotCommand::Wichteln => {
                ...
                )
            {
                ...
                if !wichtel_file.exists() {
                    ...
                    for ab in SECRETDATA.get().expect("unable to get secret data")["forbidden"]
                        .as_array()
                        .expect("forbidden not arrays")
                    {
                        forbidden.insert((
                            ab[0].as_str().unwrap().to_string(),
                            ab[1].as_str().unwrap().to_string(),
                        ));
                    }
                    ...
                    for (wichtel_from, wichtel_to) in wichtel.iter() {
                        let receiver = format!(
                            ...
                            chat_ids
                                .get(wichtel_from)
                                .expect("No chat id for wichtel_from")
                                .as_number()
                                .expect("chat_id target was not a number")
                        );
                        bot.send_message(receiver, format!("Du wichtelst fuer {}", wichtel_to))
                            .await?;
                    }
